...
import { RegisterAuthDto, LoginAuthDto, LogoutAuthDto, MeAuthDto, RefreshAuthDto, VerifyOTPAuthDto, SendPasswordResetMailAuthDto, ResetPasswordAuthDto, ResendOTP } from './dto/auth.request.dto';
...
export class AuthService {
  ...
  async ResendRegisterOTP(dto: ResendOTP): Promise<ServiceResponse<boolean>> {
    ...
    try {
      ...
      throw new InternalServerErrorException(error.messsage || 'Unexpected error occured')
    }
  }
  ...
  async verifyRegistrationOTP(dto: VerifyOTPAuthDto): Promise<ServiceResponse<LoginResponseData>> {
    const { email, OTP } = dto;
    try {
      const cacheKey = `reg-otp-${email}`;
      const hashedOTP = await this.cacheManager.get(cacheKey);
      if (!hashedOTP) throw new UnauthorizedException('No OTP found for this user');
  ...
  async sendPasswordResetMail(dto: SendPasswordResetMailAuthDto): Promise<ServiceResponse<{ sent: boolean }>> {
    const { email } = dto;
    const user = await this.UserModel.findOne({ email });
    if (!user) throw new NotFoundException('User not found');
  ...
  async ResendPasswordResetOTP(dto: ResendOTP): Promise<ServiceResponse<boolean>> {
    const { email } = dto;
    try {
      const user = await this.UserModel.findOne({ email: email });
      if (!user) throw new NotFoundException("User Does not exist");
  ...
  async verifyPasswordResetOTP(dto: VerifyOTPAuthDto): Promise<ServiceResponse<{ verified: boolean, RESET_PASS_TOKEN: string }>> {
    const { email, OTP } = dto;
    try {
      const cacheKey = `pas-otp-${email}`;
      const hashedOTP = await this.cacheManager.get(cacheKey);
      ...
      const RESET_PASS_TOKEN = generateToken(
        ...
        this.configService.get<string>('JWT_RESET_PASS_SECRET')!, this.configService.get<string>('JWT_RESET_PASS_EXPIRY')!
      );
      return {
  ...
  async ResetPassword(dto: ResetPasswordAuthDto): Promise<ServiceResponse<{ reset: boolean }>> {
